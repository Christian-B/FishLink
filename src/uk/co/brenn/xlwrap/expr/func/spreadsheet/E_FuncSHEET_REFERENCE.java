 ...
 *
 */
public class E_FuncSHEET_REFERENCE extends XLExprFunction {

	/**
	 * default constructor
	 */
	public E_FuncSHEET_REFERENCE() {
	}
	...
	public XLExprValue<?> eval(ExecutionContext context) throws XLWrapException, XLWrapEOFException {
		...
		if (!(absolute instanceof CellRange))
			throw new XLWrapException("Argument " + args.get(1) + " of " + FunctionRegistry.getFunctionName(this.getClass()) + " must be a cell range reference.");
                ...
                String sheetName = value0.getValue().toString();
                CellRange localCellRange = (CellRange)absolute;
                CellRange remoteCellRange = new CellRange(localCellRange.getFileName(), sheetName, localCellRange.getColumn(), localCellRange.getRow());
                E_RangeRef remoteRef = new E_RangeRef(remoteCellRange);
                XLExprValue<?> cellValue = remoteRef.eval(context);
		return cellValue;
	}
